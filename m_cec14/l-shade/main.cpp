...
int g_k = 10, g_m; // k is the number of variables in each subproblem, m is the number of subproblems
...
void write_csv(long seed, double error){
  ...
  const std::vector<std::string> header = {"m", "function", "length", "mode",  "eva_fun", "seed", "error"};
  std::lock_guard<std::mutex> guard(fileMutex);

  const string file_name = "../m_cec14.csv";
...
int main(int argc, char **argv) {
  for (int i = 1; i < argc; ++i) 
  {
    ...
    else if (arg == "--g_function_number" && i + 1 < argc) {
      g_function_number = atoi(argv[++i]);
    }     
  }
