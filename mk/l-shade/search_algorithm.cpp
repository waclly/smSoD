...
void searchAlgorithm::evaluatePopulation(const vector<Individual> &pop, vector<Fitness> &fitness) {
  static vector<int> constants;
  if (constants.empty()) {
    constants.resize(g_m);
    for (int m = 0; m < g_m; ++m){
      constants[m] = ((max_region - min_region) * randDouble()) + min_region;
    }
  }
  for (int i = 0; i < pop_size; i++) {
      ...
      for (int m = 0; m < g_m; ++m){
        int left = m * g_k;
        all_fitness += ((pop[i][left] - constants[m]) * (pop[i][left] - constants[m]));
        for (int k = 1; k < g_k; ++k){
          all_fitness += ((pop[i][left + k] - pop[i][left + k - 1]) * (pop[i][left + k] - pop[i][left + k - 1]));
        }
      }
      fitness[i] = all_fitness;  
